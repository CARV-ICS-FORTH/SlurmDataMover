...
#SBATCH --output=Predict%j
#SBATCH --mem-per-cpu=80000M			# Amount of RAM for each core:
...
srun -s -u /home/cadisa/slurmCluster/slurm-data-manager/build/sdm -r hidri-node01\
        -gTestFeatures.npy@fast\
	-gTestLabels.npy@fast\
	-gmnistweights.mat@fast\
	-gpredict.py@fast\
	-e "python predict.py" &

srun -s -u /home/cadisa/slurmCluster/slurm-data-manager/build/sdm -r hidri-node01 -gdata.mat@fast  -e "python predict.py"    &
